...
test('large catalogs reuse one signature per photo across allowed variants', async () => {
  ...
  assert.equal(new URL(image.thumbnail.jpeg[0].url).searchParams.get('sig'), new URL(image.lightbox.jpeg[2].url).searchParams.get('sig'));
  assert.equal(JSON.stringify(body).includes('objectKey'), false);
  clearPhotoCatalogCache();
});

test('variant whitelist accepts presets and rejects arbitrary dimensions', () => {
  const valid = new URLSearchParams(`mode=thumbnail&w=768&q=76&fmt=jpeg&fit=cover&v=demo-1&exp=2000000000&ref=${opaqueRef}`);
  const invalid = new URLSearchParams(`mode=thumbnail&w=769&q=76&fmt=jpeg&fit=cover&v=demo-1&exp=2000000000&ref=${opaqueRef}`);
  const modernFormat = new URLSearchParams(`mode=thumbnail&w=768&q=76&fmt=webp&fit=cover&v=demo-1&exp=2000000000&ref=${opaqueRef}`);
  assert.equal(normalizeVariant(valid).width, 768);
  assert.equal(normalizeVariant(invalid), null);
  assert.equal(normalizeVariant(modernFormat), null);
});
...
test('Cloudflare Images receives MIME output formats', async () => {
  ...
  assert.deepEqual(outputOptions, { format: 'image/jpeg', quality: 76 });
});

test('signatures reject expiry and tampering', async () => {
  const now = 2_000_000_000;
  const variant = { mode: 'thumbnail', width: 768, quality: 76, format: 'jpeg', fit: 'cover', version: 'demo-1', exp: now + 300, ref: opaqueRef };
...
test('image route rejects expired signatures, tampered variants, and illegal paths', async () => {
  const now = Math.floor(Date.now() / 1000);
  const assetId = 'a7f81d2e-94c6-4b37-b815-3e60e1d9af42';
  const valid = { mode: 'thumbnail', width: 768, quality: 76, format: 'jpeg', fit: 'cover', version: 'demo-1', exp: now + 300, ref: opaqueRef };
