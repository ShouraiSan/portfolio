...
test('variant whitelist accepts presets and rejects arbitrary dimensions', () => {
  const valid = new URLSearchParams('mode=thumbnail&w=768&q=76&fmt=webp&fit=cover&v=demo-1&exp=2000000000');
  const invalid = new URLSearchParams('mode=thumbnail&w=769&q=76&fmt=webp&fit=cover&v=demo-1&exp=2000000000');
...
test('signatures reject expiry and tampering', async () => {
  ...
  const variant = { mode: 'thumbnail', width: 768, quality: 76, format: 'webp', fit: 'cover', version: 'demo-1', exp: now + 300 };
...
test('unknown signed asset does not disclose internal paths', async () => {
  ...
  const query = new URLSearchParams({ mode: variant.mode, w: String(variant.width), q: String(variant.quality), fmt: variant.format, fit: variant.fit, v: variant.version, exp: String(variant.exp), sig });
...
test('image route rejects expired signatures, tampered variants, and illegal paths', async () => {
  const now = Math.floor(Date.now() / 1000);
  const assetId = 'a7f81d2e-94c6-4b37-b815-3e60e1d9af42';
  const valid = { mode: 'thumbnail', width: 768, quality: 76, format: 'webp', fit: 'cover', version: 'demo-1', exp: now + 300 };
